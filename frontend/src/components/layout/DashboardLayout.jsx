...
const NAV = [
  { to: "/dashboard", label: "Dashboard", icon: LayoutDashboard, end: true, roles: ["admin", "dentist", "staff"] },
  { to: "/appointments", label: "Appointments", icon: CalendarCheck, roles: ["admin", "dentist", "staff"] },
  { to: "/calendar", label: "Calendar", icon: CalendarDays, roles: ["admin", "dentist", "staff"] },
  { to: "/patients", label: "Patients", icon: Users, roles: ["admin", "dentist", "staff"] },
  { to: "/inventory", label: "Inventory", icon: Package, roles: ["admin", "staff"] },
  { to: "/revenue", label: "Revenue", icon: Wallet, roles: ["admin"] },
  { to: "/expenses", label: "Expenses", icon: Receipt, roles: ["admin"] },
  { to: "/reports", label: "Reports", icon: FileBarChart2, roles: ["admin"] },
  { to: "/schedules", label: "Doctor Schedule", icon: CalendarClock, roles: ["admin"] },
  { to: "/settings", label: "Settings", icon: SettingsIcon, roles: ["admin", "dentist", "staff"] },
  { to: "/portal", label: "My Dashboard", icon: LayoutDashboard, end: true, roles: ["patient"] },
  { to: "/portal/transactions", label: "Transactions", icon: Receipt, roles: ["patient"] },
];
    ...
    <div className="min-h-screen bg-canvas">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 lg:hidden no-print">
          ...
          />
          <Sidebar
            className="absolute inset-y-0 left-0 flex animate-fade-in"
...
function Sidebar({ className = "", onClose }) {
  ...
  return (
    <aside
      className={`w-64 flex-col bg-navy-800 no-print ${className} lg:fixed lg:inset-y-0`}
    >
      ...
      <div className="border-t border-white/10 px-5 py-3">
        <p className="text-[11px] text-navy-100/50">v1.0 · Baliuag, Bulacan</p>
      </div>
    </aside>
  );
}
  ...
  return (
    <header className="sticky top-0 z-40 flex h-16 items-center gap-3 border-b border-line bg-white/90 px-4 backdrop-blur sm:px-6 lg:px-8 no-print">
      ...
      <div className="ml-auto flex items-center gap-1.5">
        {role !== "patient" && <NotificationsBell />}
        <UserMenu
          user={user}
          onLogout={() => {
            logout();
            navigate(role === "patient" ? "/" : "/login");
          }}
        />
      </div>
    </header>
          ...
          <div className="scroll-slim max-h-96 overflow-y-auto">
            {items.length === 0 ? (
              ...
              items.map((n) => (
                ...
                >
                  <span
                    className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${Number(n.is_read) ? "bg-transparent" : "bg-lime-500"}`}
                  />
    ...
    <div className="relative" ref={ref}>
      ...
      >
        ...
        <span className="hidden text-left sm:block">
          ...
          <span className="block text-[11px] capitalize leading-tight text-muted">
            {user?.role}
          </span>
        </span>
